...
/// Owns the single ``GeotagCoordinator`` and gives CoreBluetooth state restoration a reliable launch hook.
///
...
final class AppDelegate: NSObject, UIApplicationDelegate {
    /// The one coordinator shared with the SwiftUI view tree (see `AlfaApp`).
    ...
    func application(
        ...
        if ProcessInfo.processInfo.environment["XCTestConfigurationFilePath"] == nil {
            coordinator.resumeIfPreviouslyEnabled()
        }
        return true
    }
}
